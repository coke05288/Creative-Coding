...
We developed the audio API specifically for this course; as such it is still under development. Our aim is for total consistency across platforms, but for now, there are a few features missing on the Java version, i.e. filter envelopes and the delay unit. We just haven't had time to implement these yet. And the filter is not quite as good as that one in the Web Audio API. 
